...
<CardGrid stagger>
  <Card title="Context Window Reset" icon="refresh-cw">
    Each loop iteration starts fresh. No accumulated confusion. The model reads your updated files and continues from where it left off.
  </Card>
  <Card title="State via Git" icon="git-branch">
    Progress persists through your codebase. The AI reads git history, modified files, and test results to orient itself each iteration.
  </Card>
  <Card title="Smart Exit Conditions" icon="stop-circle">
    Define when to stop: tests passing, no more TODOs, specific markers. ralph knows when the job is done.
  </Card>
  <Card title="Tool Agnostic" icon="tool">
    Works with Claude Code, OpenCode, Aider, or any CLI-based AI tool. Your choice.
  </Card>
</CardGrid>
