...
- PROGRESS.md               ← "Completed: auth, user. Next: payment"
...
### Strategy 1: Natural Exits

Let the model work until it naturally pauses. This respects the model's sense of a "unit of work."

```yaml
# ralph.config.json
exitConditions:
  waitForNaturalExit: true
```

### Strategy 2: Time-Based

Reset after a fixed duration to prevent deep degradation.
...
### Strategy 3: Progress-Based

Reset after measurable progress (tests pass, files complete, etc.).

```yaml
exitConditions:
  command: "npm test"
  # Resets after each test run
```
...
```yaml
exitConditions:
  timeout: 600000           # Hard limit: 10 minutes
  waitForNaturalExit: true  # But let it finish naturally if possible
  maxTokens: 80000          # Reset if context gets too large
```
