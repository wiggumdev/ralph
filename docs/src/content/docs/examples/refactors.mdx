...
```json
{
  "tool": "claude",
  "exitConditions": {
    "command": "grep -r 'UserService' src/ --include='*.ts' | wc -l",
    "successPattern": "^0$",
    "maxIterations": 20
  }
}
...
## Progress
Check git log for completed files.
```
...
```json
{
  ...
  "exitConditions": {
    "all": [
      { "command": "grep -r 'api/v1' src/", "successPattern": "^$" },
...
## Done When
- No references to `/api/v1/` remain
- All tests pass
```
...
```json
{
  ...
  "exitConditions": {
    "all": [
      { "command": "npm run build", "successExitCode": 0 },
      { "command": "npm test", "successExitCode": 0 }
    ],
    "maxIterations": 30
  }
...
No changes needed—React 18 handles this automatically.

### 3. Strict Mode Changes
Double-rendering in Strict Mode is now expected. Remove any workarounds.
...
- `npm test` passes
- No React 17 deprecation warnings
```
...
```json
{
  ...
  "exitConditions": {
    "all": [
      {
        "command": "grep -r \"from 'moment'\" src/ | wc -l",
        "successPattern": "^0$"
      },
      { "command": "npm test", "successExitCode": 0 }
    ],
...
- date-fns is tree-shakeable (smaller bundles)
...
## Done When
- No imports from 'moment'
- All tests pass
- moment not in package.json
```
...
2. Commit with message: "refactor(dirname): migrate to new API"
```
